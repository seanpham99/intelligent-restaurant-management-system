...
export default function Layout({ children, className = '' }: LayoutProps) {
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row items-stretch justify-center p-0">
      {/* Side Nav Rail */}
      ...
      <motion.main
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className={`w-full max-w-[390px] md:max-w-none md:flex-1 min-h-screen md:h-screen bg-background relative overflow-x-hidden flex flex-col ${className}`}
      >
        {children}
      </motion.main>
    </div>
